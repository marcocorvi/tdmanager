/* @file TdViewActivity.java
 *
 * @author marco corvi
 * @date nov 2011
 *
 * @brief TdManager main drawing activity
 * --------------------------------------------------------
...
/**
 */
public class TdViewActivity extends Activity
                                      ...
                                      // , OnItemClickListener
{
  ...
  private TdViewSurface mDrawingSurface;
  ...
  public void onVisibilityChanged(boolean visible)
  {
    ...
    ArrayList< TdViewCommand > getCommands() { return mDrawingSurface.mCommandManager; }
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
      ...
      mDrawingSurface = (TdViewSurface) findViewById(R.id.drawingSurface);
...
// ----------------------------------------------------------------------------
    ...
    {
      ArrayList< TdSurvey > surveys = mApp.mViewSurveys;
      if ( surveys == null ) return;
      // TdConfig config = mApp.mConfig;
      ArrayList< TdEquate > equates = mApp.mConfig.mEquates;

      // Log.v( "TdManager", "TdView nr. surveys " + surveys.size() + " equates " + equates.size() );
    ...
    int mWithStation = 0;
    TdViewCommand mSelectedCommand = null;
    ...
    public boolean onTouch( View view, MotionEvent rawEvent )
    {
      ...
      if (action == MotionEvent.ACTION_POINTER_DOWN) {
        ...
        // Log.v("TdManager", "POINTER DOWN old dist " + oldDist );
        doMove = false;
        ...
        mSaveX = x_canvas;
        mSaveY = y_canvas;
        // Log.v("TdManager", "POINTER UP " + mSaveX + " " + mSaveY );
      ...
      // ---------------------------------------- DOWN
        ...
        if ( mWithStation == 0 ) {
          ...
          // Log.v("TdManager", "DOWN at " + mSaveX + " " + mSaveY + " at " + ret );
          if ( ret ) {
            mWithStation = 1;
            mSelectedCommand = mDrawingSurface.selectedCommand();
            setTitle( "TdManager " + mDrawingSurface.selectedCommandName() + " " + mDrawingSurface.selectedStationName() );
          } else {
            setTitle( "TdManager" );
          }
      ...
      // ---------------------------------------- MOVE
        ...
        if ( mTouchMode == MODE_MOVE) {
          // Log.v("TdManager", "MOVE (move) to " + x_canvas + " " + y_canvas );
          ...
          // Log.v("TdManager", "MOVE (zoom) dist " + newDist );
  ...
  //     // (new TdViewEquateDialog( this, this, mApp ) ).show(); 

  //     // this dialog shows equates
  //     (new TdEquatesDialog( this, mApp.mConfig, this )).show();
  ...
  private void handleEquate()
  {
    if ( mSelectedCommand == null ) {
      ...
      // manually add equate
      new TdEquateNewDialog( this, this, mDrawingSurface.mCommandManager ).show();
    } else {
      // TdViewCommand cmd1 = mSelectedCommand;
      // TdSurvey srv1 = cmd1.mSurvey;
      TdViewStation vst1 = mSelectedCommand.mSelected;
      // TdStation stn1 = vts1.mStation;
      float x = vst1.x + mSelectedCommand.mXoff;
      float y = vst1.y + mSelectedCommand.mYoff;
      // Log.v("TdManager", "selected station " + vst1.x + " " + vst1.y + " point " + x + " " + y );
      ...
      if ( mDrawingSurface.getSurveyAt( x, y, mSelectedCommand ) ) {
        // TdViewCommand cmd2 = mDrawingSurface.selectedCommand();
        // TdSurvey srv2 = cmd2.mSurvey;
        // TdViewStation vst2 = mDrawingSurface.selectedStation();
        // TdStation stn2 = vts2.mStation;
        ...
        new TdAlertDialog( this, mApp.getResources(), title, 
  ...
  void makeEquate( String st1, String st2 )
  {
    ...
    // Log.v("TdManager", "add equate: " + equate.stationsString() );
